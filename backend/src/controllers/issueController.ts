...
const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
        ...
        if (file.mimetype.startsWith("image/")) {
            ...
            console.log(
                "[Multer] Rejected file - not an image:",
                file.mimetype
            );
    ...
    try {
        ...
        if (req.file) {
            ...
            try {
                ...
                console.error(
                    "[CreateIssue] Image upload failed:",
                    uploadError
                );
                res.status(400).json({
                    success: false,
        ...
        if (req.io) {
            ...
            const assignmentResult = await assignmentService.autoAssignIssue(
                issue
            );
...
export const updateIssueStatus = async (
    ...
    try {
        ...
        const issue = await IssueReport.findByIdAndUpdate(id, updateData, {
            new: true,
        })
            .populate("reportedBy", "firstName lastName email role")
            .populate("assignedTo", "firstName lastName email role")
