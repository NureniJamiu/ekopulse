...
export const getAllAgencies = async (
    ...
    try {
        ...
        const agencies = await Agency.find(query).sort({
            priority: 1,
            name: 1,
        });
...
export const getAgencyIssues = async (
    req: AuthenticatedRequest,
    res: Response
): Promise<void> => {
    try {
...
export const getMyAgencyIssues = async (
    ...
    try {
        ...
        const query: any = {
            assignedAgency: new mongoose.Types.ObjectId(
                agency._id?.toString() || ""
            ),
        };
        ...
        if (agency.issueTypes && agency.issueTypes.length > 0) {
            if (type && agency.issueTypes.includes(type as string)) {
                ...
                console.log(
                    "[getMyAgencyIssues] Requested type not supported by agency:",
                    type
                );
                res.status(200).json({
                    success: true,
        ...
        if (issues.length > 0) {
            console.log(
                "[getMyAgencyIssues] Sample issue types:",
                issues.slice(0, 3).map((i) => i.type)
            );
        }
...
export const getMyAgencyStats = async (
    req: AgencyAuthenticatedRequest,
    res: Response
): Promise<void> => {
    try {
...
export const assignIssueToAgency = async (
    req: AuthenticatedRequest,
    res: Response
): Promise<void> => {
    try {
        ...
        if (req.io) {
            ...
            const result = await assignmentService.manualAssignIssue(
                issueId,
                agencyId,
                userId
            );
...
export const registerAgency = async (
    ...
    try {
        ...
        const agencyId = `AG${Date.now()}${Math.random()
            .toString(36)
            .substr(2, 4)
            .toUpperCase()}`;
...
export const setupAgencyCredentials = async (
    req: Request,
    res: Response
): Promise<void> => {
    try {
...
export const loginAgency = async (
    req: Request,
    res: Response
): Promise<void> => {
    try {
...
export const verifyAgencySession = async (
    req: Request,
    res: Response
): Promise<void> => {
    try {
...
async function sendAgencySetupEmail(agency: IAgency): Promise<void> {
    try {
        ...
        }/agency-setup?token=${agency.setupToken}`;
    ...
    `;
        ...
        console.log(
            "[Email] Agency can use either the email link or the console backup link above"
        );
    } catch (error) {
        console.error("[Email] Failed to send setup email:", error);
        console.log(
            "[Email] Email delivery failed, but backup link is available in console above"
        );
...
export const debugAgencyIssues = async (
    req: Request,
    res: Response
): Promise<void> => {
    try {
        ...
        const agencies = await Agency.find({}).select(
            "name type issueTypes isActive status"
        );
        ...
        const debugInfo = {
            ...
            agencies: agencies.map((agency) => {
                const agencyIssues = issues.filter(
                    (issue) =>
                        issue.assignedAgency &&
                        issue.assignedAgency._id?.toString() ===
                            agency._id?.toString()
                );
                ...
                return {
                    ...
                    assignedIssueTypes: [
                        ...new Set(agencyIssues.map((i) => i.type)),
                    ],
                    typeMismatches: agencyIssues
                        .filter(
                            (issue) =>
                                !agency.issueTypes ||
...
export const updateIssueStatusByAgency = async (
    req: AgencyAuthenticatedRequest,
    res: Response
): Promise<void> => {
    try {
        ...
        if (
            !issue.assignedAgency ||
            issue.assignedAgency.toString() !== agency._id?.toString()
        ) {
            res.status(403).json({
                success: false,
        ...
        if (req.io) {
            ...
            req.io
                .to(`issue_${updatedIssue._id}`)
                .emit("issue_status_updated", updatedIssue);
        }
