...
export class AgencyAssignmentService {
    ...
    async autoAssignIssue(issue: IIssueReport): Promise<AssignmentResult> {
        try {
            console.log(
                "[AgencyAssignmentService] Starting auto-assignment for issue:",
                {
                    id: issue._id,
                    type: issue.type,
            ...
            if (suitableAgencies.length === 0) {
                console.log(
                    "[AgencyAssignmentService] No suitable agencies found"
                );
                return {
                    success: false,
            ...
            const selectedAgency = this.selectBestAgency(
                suitableAgencies,
                issue
            );
            console.log(
                "[AgencyAssignmentService] Selected agency:",
                selectedAgency.name
            );
            ...
            if (!updatedIssue) {
                console.log(
                    "[AgencyAssignmentService] Failed to update issue assignment"
                );
                return {
                    success: false,
            ...
            console.log(
                "[AgencyAssignmentService] Issue successfully assigned to agency"
            );

            // Create notification for the agency using NotificationService
            await this.notificationService.notifyIssueAssignment(
                updatedIssue,
                selectedAgency
            );
            ...
            this.io
                .to(`agency_${selectedAgency._id}`)
                .emit("issue_assigned", updatedIssue);
            this.io
                .to(`issue_${issue._id}`)
                .emit("issue_updated", updatedIssue);
     ...
     */
    private async findSuitableAgencies(
        issue: IIssueReport
    ): Promise<IAgency[]> {
        const query: any = {
            isActive: true,
        ...
        console.log(
            "[AgencyAssignmentService] Query for finding suitable agencies:",
            JSON.stringify(query, null, 2)
        );

        const agencies = await Agency.find(query).sort({ priority: 1 });
        console.log(
            "[AgencyAssignmentService] Found suitable agencies:",
            agencies.length,
