...
import { Shield, TrendingUp, Clock, CheckCircle, AlertCircle } from 'lucide-react';
...
const AuthorityDashboardPage: React.FC = () => {
  ...
  const handleStatusUpdate = async (issueId: string, newStatus: string) => {
    try {
      const updatedIssue = await issuesAPI.updateIssueStatus(issueId, newStatus);
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {Object.keys(stats).map((status) => (
          ...
          >
            {status === 'total' ? 'All Reports' : status.replace('_', ' ').toUpperCase()}
      ...
      {filteredIssues.length === 0 ? (
        <EmptyState
          ...
          description={`No ${filter === 'total' ? '' : filter.replace('_', ' ')} issues to display.`}
        />
      ) : (
