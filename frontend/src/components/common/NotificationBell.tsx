...
interface Notification {
  ...
  title: string;
  message: string;
  type: 'issue_assigned' | 'status_updated' | 'new_report' | 'agency_mention';
...
const NotificationBell: React.FC<NotificationBellProps> = ({ recipientType = 'user' }) => {
  const { user } = useAuth();
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      case 'issue_assigned': return <FileText className="w-4 h-4 text-blue-500" />;
      case 'status_updated': return <Check className="w-4 h-4 text-green-500" />;
      case 'new_report': return <AlertCircle className="w-4 h-4 text-orange-500" />;
