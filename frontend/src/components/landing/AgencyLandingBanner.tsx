...
const AgencyLandingBanner: React.FC = () => {
    ...
    const { isAuthenticated: isAgencyAuthenticated } = useAgencyAuth();
    const [isVisible, setIsVisible] = useState(true);

    // Don't show banner if user is already signed in (either as citizen via Clerk or as agency)
    if (isSignedIn || isAgencyAuthenticated || !isVisible) return null;
                    ...
                    <div>
                        ...
                        <div className="space-y-3 mb-6">
                            ...
                            <div className="flex items-center gap-3">
                                <CheckCircle className="w-5 h-5 text-green-300" />
                                <span>
                                    Automated issue assignment based on your
                                    service areas
                                </span>
                            </div>
                            <div className="flex items-center gap-3">
                                <CheckCircle className="w-5 h-5 text-green-300" />
                                <span>
                                    Track and update issue resolution status
                                </span>
                            </div>
                            <div className="flex items-center gap-3">
                                <CheckCircle className="w-5 h-5 text-green-300" />
                                <span>
                                    Connect directly with citizens and other
                                    agencies
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="bg-white/10 rounded-xl p-6 backdrop-blur-sm">
                            ...
                            <div className="mt-4 pt-4 border-t border-white/20">
                                <div className="flex items-center gap-2 text-sm text-blue-100">
                                    ...
                                    <span>
                                        Operating in 12+ cities nationwide
                                    </span>
                                </div>
                            </div>
                        </div>
