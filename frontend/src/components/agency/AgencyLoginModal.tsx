...
const AgencyLoginModal: React.FC<AgencyLoginModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('[Agency Login] Attempting login with:', { agencyId: formData.agencyId });
      ...
      if (response.ok) {
        ...
        toast.error(errorData.error || 'Invalid credentials. Please check your Agency ID and password.');
      }
    } catch (error) {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold text-gray-900">Agency Login</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="relative">
              <input
                ...
                className={`w-full px-4 py-3 pl-11 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                  errors.agencyId ? 'border-red-500' : 'border-gray-300'
                }`}
                placeholder="Enter your unique Agency ID"
                disabled={loading}
              />
          ...
          <div className="text-center text-xs text-gray-500">
            ...
            <p>Your Agency ID and setup link were sent to your email after registration.</p>
          </div>
        </form>
      </div>
    </div>
