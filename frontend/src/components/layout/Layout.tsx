...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const navigation = useMemo(() => {
    const baseNavigation = [
      { name: 'Map', href: '/', icon: MapPin, current: window.location.pathname === '/' },
    ];

    // Add role-specific navigation
    if (isSignedIn) {
      // For Clerk authenticated users
      baseNavigation.push({ name: 'My Reports', href: '/my-reports', icon: FileText, current: window.location.pathname === '/my-reports' });
      ...
      if (isAuthority) {
        baseNavigation.push({
          ...
          icon: Shield,
          current: window.location.pathname === '/authority-dashboard'
        });
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="flex justify-between h-16">
              ...
              <div className="flex items-center">
                <a href="/" className="flex-shrink-0 flex items-center hover:opacity-80 transition-opacity">
                  ...
                  <span className="ml-2 text-xl font-bold text-gray-900">EcoPulse</span>
                </a>
                  ...
                  {(user || agency) && (
                    <span className={`hidden sm:inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      ...
                      {user?.role === 'authority' ? 'Authority' :
                       user?.role === 'agency_admin' || agency ? 'Agency' : 'Citizen'}
                    </span>
                  )}
      ...
      {/* Main Content */}
      <main className="flex-1" style={{ minHeight: isAuthPage ? '100vh' : 'calc(100vh - 64px)' }}>
        {children}
      </main>
